...
public class XMLSchemaUpdater {
	...
	/**
	 * Reads the given xml schema (resource) and searches for included and imported
	 * schemas in the file. If these files are local, the function tries to copy the
	 * resources into a new directory next to the given schema (resource) and adapts
	 * the dependencies in the resource. The oldFile is the path of the xml schema
	 * before it was copied to his new directory (eg. temporary directory). The
	 * oldFile keeps untouched. Resource has to be a copy of oldFile. <br>
	 * <br>
	 * Example:<br>
	 ...
	 * So hydro.xsd is copied into 'C:/Local/Temp/1348138164029-0/watercourse/' (or
	 ...
	 * @param includeWebResources true if web resources should be copied and updated
	 *            too otherwise false
	 * @param reporter the reporter of the current I/O process where errors should
	 *            be reported to
