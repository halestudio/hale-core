...
public final class ContentTypeCatalog {
	...
	private int collectMatchingByContents(int valid, IContentType[] subset,
			List<ContentType> destination, ILazySource contents, Map<String, Object> properties)
			throws IOException {
	...
	synchronized void dissociate(ContentType contentType, String text, int type) {
		Map<String, Set<ContentType>> fileSpecMap = ((type & IContentType.FILE_NAME_SPEC) != 0)
				? fileNames
				: fileExtensions;
		String mappingKey = FileSpec.getMappingKeyFor(text);
		Set<ContentType> existing = fileSpecMap.get(mappingKey);
	...
	IContentType[] findContentTypesFor(ContentTypeMatcher matcher, InputStream contents,
	...
	IContentType[] findContentTypesFor(ContentTypeMatcher matcher, final String fileName) {
		IContentType[] selected = concat(
				internalFindContentTypesFor(matcher, fileName, policyConstantGeneralIsBetter));
		// give the policy a chance to change the results
		ISelectionPolicy policy = matcher.getPolicy();
		if (policy != null)
	...
	private IContentDescription getDescriptionFor(ContentTypeMatcher matcher, ILazySource contents,
		...
		return matcher.getSpecificDescription(
				((ContentType) selected[0]).internalGetDescriptionFor(contents, options));
	}

	public IContentDescription getDescriptionFor(ContentTypeMatcher matcher, InputStream contents,
			String fileName, QualifiedName[] options) throws IOException {
		return getDescriptionFor(matcher, ContentTypeManager.readBuffer(contents), fileName,
				options);
	}

	public IContentDescription getDescriptionFor(ContentTypeMatcher matcher, Reader contents,
			String fileName, QualifiedName[] options) throws IOException {
		return getDescriptionFor(matcher, ContentTypeManager.readBuffer(contents), fileName,
				options);
	}
	...
	private IContentType[] internalFindContentTypesFor(ILazySource buffer, IContentType[][] subset,
			Comparator<IContentType> validPolicy, Comparator<IContentType> indeterminatePolicy)
			throws IOException {
		...
		final int validExtension = collectMatchingByContents(validFullName, subset[1], appropriate,
				buffer, properties) - validFullName;
		final int appropriateExtension = appropriate.size() - appropriateFullName;
		final int validPattern = collectMatchingByContents(validExtension, subset[2], appropriate,
				buffer, properties) - validExtension;
		final int appropriatePattern = appropriate.size() - appropriateFullName
		...
		if (validPattern > 1) {
			Arrays.sort(result, validFullName + validExtension,
					validFullName + validExtension + validPattern, validPolicy);
		}
		if (appropriateFullName - validFullName > 1)
			Arrays.sort(result, validFullName + validExtension,
					appropriateFullName + validExtension, indeterminatePolicy);
		if (appropriateExtension - validExtension > 1)
			Arrays.sort(result, appropriateFullName + validExtension,
					appropriate.size() - validPattern, indeterminatePolicy);
		if (appropriatePattern - validPattern > 1) {
			Arrays.sort(result, appropriate.size() - validPattern, appropriate.size(),
					indeterminatePolicy);
		}
		return result;
	}

	private IContentType[] internalFindContentTypesFor(ContentTypeMatcher matcher,
			ILazySource buffer, String fileName, boolean forceValidation) throws IOException {
		...
		if (!forceValidation && total == 1) {
			// do not do validation if not forced and only one was found (caller will
			// validate later)
			IContentType[] found = subset[0].length == 1 ? subset[0]
					: (subset[1].length == 1 ? subset[1] : subset[2]);
	 ...
	 */
	synchronized private IContentType[][] internalFindContentTypesFor(ContentTypeMatcher matcher,
			final String fileName, Comparator<IContentType> sortingPolicy) {
		...
		if (context.equals(manager.getContext()))
			...
			allByFileName.addAll(matcher.getDirectlyAssociated(this, fileName,
					IContentTypeSettings.FILE_NAME_SPEC));
		}
		Set<ContentType> selectedByName = selectMatchingByName(context, allByFileName,
		...
		if (fileExtension != null) {
			...
			if (context.equals(manager.getContext()))
				allByFileExtension = getDirectlyAssociated(fileExtension,
				...
				allByFileExtension.addAll(matcher.getDirectlyAssociated(this, fileExtension,
						IContentTypeSettings.FILE_EXTENSION_SPEC));
			}
			Set<ContentType> selectedByExtension = selectMatchingByName(context, allByFileExtension,
			...
			if (!selectedByExtension.isEmpty())
				result[1] = selectedByExtension
						.toArray(new IContentType[selectedByExtension.size()]);
		}
		...
		if (context.equals(manager.getContext()))
			allByFilePattern = getMatchingRegexpAssociated(fileName,
			...
			allByFilePattern.addAll(matcher.getMatchingRegexpAssociated(this, fileName,
					IContentTypeSettings.FILE_PATTERN_SPEC));
		}
	...
	private Set<ContentType> getMatchingRegexpAssociated(String fileName, int typeMask) {
		...
		for (Entry<Pattern, Set<ContentType>> spec : this.fileRegexps.entrySet()) {
			if (spec.getKey().matcher(fileName).matches()) {
				res.addAll(filterOnDefinitionSource(initialPatternForRegexp.get(spec.getKey()),
						typeMask, spec.getValue()));
			}
		}
		return res;
	}
	...
	private Set<ContentType> getDirectlyAssociated(String text, int typeMask) {
		...
		Map<String, Set<ContentType>> associations = (typeMask
				& IContentTypeSettings.FILE_NAME_SPEC) != 0 ? fileNames : fileExtensions;
		Set<ContentType> result = associations.get(FileSpec.getMappingKeyFor(text));
		if ((typeMask
				& (IContentType.IGNORE_PRE_DEFINED | IContentType.IGNORE_USER_DEFINED)) != 0) {
	 ...
	 * provided criteria.
	 *
	 * @param text file name, file extension or file regexp (depending on value of
	 *            {@code typeMask}.
	 * @param typeMask the type mask. Spec type, and definition source (pre-defined
	 *            or user-defined) will be used
	 * @param contentTypes content types to filter from (not modified during method
	 *            execution)
	 * @return set of filtered content-type
	 */
	private Set<ContentType> filterOnDefinitionSource(String text, int typeMask,
	...
	/**
	 * Processes all content types in source, adding those matching the given file
	 * spec to the destination collection.
	 */
	private Set<ContentType> selectMatchingByName(final IScopeContext context,
			Collection<ContentType> source, final Collection<ContentType> existing,
			final String fileSpecText, final int fileSpecType) {
			...
			internalAccept(new ContentTypeVisitor() {
				...
				public int visit(ContentType type) {
					...
					if (type == root && !type.hasFileSpec(context, fileSpecText, fileSpecType))
						// it is the root and does not match the file name - do not add it nor look
						// into its children
						return RETURN;
