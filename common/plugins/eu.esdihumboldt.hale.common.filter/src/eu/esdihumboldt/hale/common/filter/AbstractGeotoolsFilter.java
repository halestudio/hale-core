...
public abstract class AbstractGeotoolsFilter
	...
	public Optional<eu.esdihumboldt.hale.common.instance.model.Filter> migrateFilter(
		...
		for (Filter part : filterParts) {
			...
			/*
			 * Determine if part is relevant. Only accept filter parts that are not
			 * exclusively updated with other types than `preferRoot`. (This is used to
			 * handle the different types from a Join individually, also for properties that
			 * are mapped in the same context)
			 *
			 * TODO is usage of preferRoot OK or should we have an additional parameter to
		...
		switch (splitType) {
		case AND:
			combined = CommonFactoryFinder.getFilterFactory(null).and(acceptedParts);
			break;
		case OR:
			combined = CommonFactoryFinder.getFilterFactory(null).or(acceptedParts);
			break;
		default:
	...
	public String toString() {
		return getClass().getSimpleName() + "{" + "filterTerm='" + getFilterTerm() + '\'' + '}';
	}
}
