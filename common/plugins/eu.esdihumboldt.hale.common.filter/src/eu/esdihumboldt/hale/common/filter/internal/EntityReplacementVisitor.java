...
public class EntityReplacementVisitor extends DuplicatingFilterVisitor {
	...
	/**
	 * Determine if related to an expected parent type all replacement attempts were
	 * mismatches or matching entities with a different parent type.
	 *
	 * @param expectedParent the expected parent type
	 * @return <code>true</code> if related to the given expected parent type all
	 *         replacement attempts were mismatches or matching entities with a
	 *         different parent type, <code>false</code> if there were successful
	 *         matches with the given parent type
	 */
