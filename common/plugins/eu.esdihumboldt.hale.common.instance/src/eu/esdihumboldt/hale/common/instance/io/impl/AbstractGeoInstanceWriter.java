...
public abstract class AbstractGeoInstanceWriter extends AbstractInstanceWriter
	...
	/**
	 * Maximum number of points a geometry may have for its full WKT representation
	 * to be built for an error message. Beyond this, building the WKT text is
	 * skipped and a cheap description is used instead, to avoid materializing a
	 * potentially huge string only to truncate it.
	 */
	private static final int WKT_EXCERPT_MAX_POINTS = 200;
	...
	private static String toWktExcerpt(Geometry geom) {
		...
		try {
			if (geom.getNumPoints() > WKT_EXCERPT_MAX_POINTS) {
				// avoid materializing the full WKT of a large geometry just to
				// truncate it again afterwards
				return describeLargeGeometry(geom);
			}
	...
	/**
	 * Describe a geometry that is too large to include a WKT representation of in
	 * an error message, using cheap-to-compute properties instead.
	 *
	 * @param geom the geometry to describe
	 * @return the description
	 */
	private static String describeLargeGeometry(Geometry geom) {
		StringBuilder sb = new StringBuilder();

		sb.append(geom.getGeometryType()).append(" with ").append(geom.getNumPoints())
				.append(" points");
